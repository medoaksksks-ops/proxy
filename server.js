...
async function getRecommendedVideos(region = 'EG', limit = 20) {
  ...
  // 1) القنوات المفضّلة — دايمًا أولوية، بناخد منها لحد 65% من العدد المطلوب
  try {
    const favItems = await getFavoriteChannelsContent(Math.ceil(limit * 0.65));
    items.push(...favItems);
  } catch (e) {
    log.warn(`Favorite channels fetch failed: ${e.message}`);
  }

  // 2) نكمّل الباقي من تبويب الرائج العام
  if (items.length < limit) {
    try {
      ...
      items.push(...filterClean(parseFlatItems(stdout)));
    } catch (e) {
      log.warn(`Trending feed failed (${e.message}), falling back to search-based mix`);
    }
  }

  // 3) لسه ناقص -> مواضيع عامة متنوعة (آخر حل بس)
  if (items.length < 5) {
