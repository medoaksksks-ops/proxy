...
fs = require(‘fs’); const path = require(‘path’); // (مش محتاجين مكتبة
cors تاني، الهيدرز بقت بتتحط يدوي فوق) const NodeCache =
require(‘node-cache’); const https = require(‘https’);
require(‘dotenv’).config();

//
==========================================================================
// 🔖 srver v5.0.0 “جبارة” — نسخة موسّعة فوق v4.0 الأصلية بدون حذف أي
حاجة: // • كل جودات الفيديو (144p → 4K) + دمج فيديو/صوت لحظي بـ ffmpeg
للجودات // العالية اللي معندهاش progressive stream جاهز. // • هوم فيد
بأقسام (Sections) زي صفحة يوتيوب الرئيسية الحقيقية. // • جلب متوازي
(Promise.all) بدل التسلسلي → أسرع بشكل ملحوظ. // • keep-alive agent
لإعادة استخدام الاتصالات مع جوجل. //
==========================================================================
const SERVER_VERSION = ‘7.1.0’;

// Agent واحد بيعيد استخدام نفس اتصالات TCP/TLS بدل ما يفتح اتصال جديد
لكل // طلب لجوجل — ده اللي بيدي إحساس “سريع” فعلي في البث والـ API calls
...
// Firebase config const FIREBASE_URL = process.env.FIREBASE_URL ||
‘https://english-73376-default-rtdb.firebaseio.com’; const
FIREBASE_SECRET = process.env.FIREBASE_SECRET || ’’;

// CORS intentionally removed: same-origin deployment avoids browser
preflight overhead.

app.use(express.json()); app.use(express.text({ limit: ‘10mb’ }));

// —————- Cache —————- const infoCache = new NodeCache({ stdTTL: 10800
}); // معلومات فيديو/بحث/related: ساعتين const trendingCache = new
NodeCache({ stdTTL: 600 }); // الرائج: 20 دقيقة const streamCache = new
NodeCache({ stdTTL: 300 }); // روابط التشغيل المباشرة بتنتهي بسرعة: 4
دقايق بس const channelCache = new NodeCache({ stdTTL: 7200 }); // بيانات
وفيديوهات القنوات: ساعة

const TIMEOUT = 45000; const MAX_RETRIES = 2;

// Logger const log = { info: (msg) =>
...
//
==========================================================================
// Concurrency limiter — بيمنع الـ Railway instance من إنه يتحمّل أكتر من
طاقته // (كل عملية yt-dlp بتاخد وقت وذاكرة، فمينفعش نسيب عدد لا نهائي
يشتغلوا مع بعض) //
==========================================================================
...
// منع تشغيل نفس yt-dlp أكثر من مرة لو عدة مستخدمين طلبوا نفس الشيء في
نفس اللحظة. const inflight = new Map(); function dedupe(key, fn) { if
...
//
==========================================================================
// كوكيز يوتيوب — بيتحدّثوا في الخلفية كل 5 دقايق بدل ما كل request يعمل
طلب // لـ Firebase لوحده (كان بيسبب race condition وبطء ومكالمات مكررة
كتير) //
==========================================================================
...
// Check if yt-dlp is installed function checkYtDlp() { try {
require(‘child_process’).execSync(‘yt-dlp –version’, { stdio: ‘ignore’
}); return true; } catch { return false; } }

// Validation function isValidVideoId(id) { return /[1]{11}$/.test(id);
}

function sanitizeFilename(name) { return name.replace(/[^\w\s-]/g,
’‘).substring(0, 100) || ’video’; }

/** * تشغيل yt-dlp بشكل غير متزامن (async) بدون shell — بيستقبل الـ args
كمصفوفة * عشان محدّش يقدر يحقن أوامر شل حتى لو query البحث فيه رموز
غريبة، وكمان * بيدي كل request مكانه في الطابور (semaphore) بدل ما يبوّظ
السيرفر كله. */ function commandExists(command) { try {
...
String(error?.message || error || ’’).toLowerCase(); return /page needs
to be reloaded|sign in to confirm|confirm you’re not a bot|confirm
you’re not a bot|http error 403|requested format is not available|video
unavailable|not available in your country/.test(text); }
...
==========================================================================
*/ const UNWANTED_KEYWORDS = [ // أطفال / كرتون ‘كرتون’, ‘رسوم متحركة’,
‘للأطفال’, ‘اطفال’, ‘أطفال’, ‘بيبي’, ‘بيبى’, ‘روضة’, ‘حضانة’, ‘قصص
اطفال’, ‘قصص أطفال’, ‘اغاني اطفال’, ‘أغاني أطفال’, ‘العاب اطفال’, ‘ألعاب
أطفال’, ‘تعليم اطفال’, ‘تعليم أطفال’, ‘انمي اطفال’, ‘مسلسل كرتون’,
‘cartoon’, ‘kids’, ‘for kids’, ‘nursery rhyme’, ‘nursery rhymes’,
‘cocomelon’, ‘baby shark’, ‘peppa pig’, ‘toddler’, ‘preschool’,
‘children song’, // طبخ / وصفات ‘وصفة’, ‘وصفات’, ‘طبخ’, ‘طبخة’, ‘طريقة
عمل’, ‘حلويات’, ‘أكلة’, ‘اكلة’, ‘مطبخ’, ‘شيف’, ‘recipe’, ‘cooking’,
...
جديدة * من غير ما يعيد طلب yt-dlp لنفس البيانات القديمة تاني. / async
function getPaginatedPool(cache, cacheKeyBase, fetchPoolFn, page, limit,
...
==========================================================================
*/ const FOLLOWED_CREATORS = [ ‘كامل العربي’, ‘اوشا’, ‘صلاح القصة وما
فيها’, ‘سامح سند’, ‘بدر العلوي’, ‘ابو الصادق’, ‘مستر محمد ايمن الجوهري’,
‘مستر محمد صلاح مدرس لغة انجليزية’, ‘مستر محمد عبدالمعبود’, ‘مستر رضا
الفاروق’, ‘انجلشاوي’, ‘عبقري لغة خالد صقر’, ‘قناة توست’, ‘كوتش
الغلابة’];

const DISCOVERY_BLOCKED = [ ‘shorts’, ‘#shorts’, ‘ميمز’, ‘مقاطع مضحكة
جدا’, …UNWANTED_KEYWORDS];
...
const DISCOVERY_QUERIES = [ ‘أخبار مصر اليوم’, ‘ترند مصر اليوم’, ‘أهم
الأخبار اليوم مصر’, ‘تكنولوجيا اليوم مراجعات’, ‘كرة القدم مصر اليوم’,
...
==========================================================================
*/ const HOME_SECTIONS = [ { key: ‘trending’, title: ‘🔥 الرائج الآن’,
query: null }, // بيتجاب من فيد الترند الحقيقي { key: ‘music’, title:
‘🎵 موسيقى’, query: ‘أغاني عربي جديد 2026’ }, { key: ‘sports’, title:
‘⚽ رياضة’, query: ‘أهداف وملخصات مباريات’ }, { key: ‘gaming’, title:
‘🎮 ألعاب’, query: ‘ألعاب فيديو جيمنج’ }, { key: ‘news’, title: ‘📰
أخبار’, query: ‘أخبار عاجلة اليوم’ }, { key: ‘tech’, title: ‘💻
تكنولوجيا’, query: ‘تكنولوجيا مراجعات تقنية’ }, { key: ‘entertainment’,
title: ‘🎬 ترفيه وأفلام’, query: ‘أفلام ومسلسلات تريلر’ }, { key:
‘podcasts’, title: ‘🎙️ بودكاست’, query: ‘بودكاست عربي حوار’ }, { key:
‘comedy’, title: ‘😂 كوميدي’, query: ‘فيديوهات كوميدي مضحكة’ }, { key:
‘live’, title: ‘🔴 مباشر الآن’, query: ‘بث مباشر live’ }];
...
stdout.trim().split(‘’).filter(Boolean); const data =
JSON.parse(lines[lines.length - 1]); return (data.comments ||
[]).slice(0, limit).map(c => ({ id: c.id, author: c.author || ‘مستخدم
يوتيوب’, authorThumbnail: c.author_thumbnail || ’‘, text: c.text ||’‘,
likeCount: c.like_count || 0, isReply: !!(c.parent && c.parent !==
’root’), timestamp: c.timestamp ? new Date(c.timestamp
...
/** * GET /search?q=QUERY&limit=20&page=1 */ app.get(‘/search’, async
(req, res) => { const { q: query } = req.query;
...
if (!channelId) { return res.status(400).json({ error: ‘channel id
مطلوب’, example: ‘/channel?id=UCxxxxxxxx&limit=20&page=1’ }); }
...
/** * GET /comments?v=VIDEO_ID&limit=50 */ app.get(‘/comments’, async
(req, res) => { const videoId = req.query.v; const limit =
...
res.status(500).json({ error: ‘تعذّر جلب التعليقات (ممكن تكون التعليقات
مقفولة على الفيديو ده)’, details: NODE_ENV === ‘development’ ?
...
const headers = { ‘User-Agent’: ‘Mozilla/5.0 (Windows NT 10.0; Win64;
x64) AppleWebKit/537.36 Chrome/120 Safari/537.36’, ‘Accept’: ‘/’,
‘Accept-Encoding’: ‘identity’ }; if (req.headers.range) headers[‘Range’]
= req.headers.range;

const upstreamReq = https.get(url, { headers, timeout: 20000, agent:
keepAliveAgent }, (upstreamRes) => { // تتبّع أي redirect إضافي بنفسنا
(مش بنسيبه للمتصفح) if ([301, 302, 303, 307,
...
upstreamReq.on(‘timeout’, () => upstreamReq.destroy(new Error(‘Upstream
timeout’))); upstreamReq.on(‘error’, (err) => {
...
//
==========================================================================
// نظام الجودات v6: // - نرجع الارتفاعات الموجودة فعليًا فقط. // -
progressive بنفس الارتفاع = تشغيل مباشر وأسرع. // - video-only + audio =
ffmpeg فقط عند الحاجة. //
==========================================================================
...
getVideoFormats(info); if (!formats.length) throw new Error(‘No video
formats available’);
...
// FFmpeg لما يطلع fragmented MP4 على pipe بيحط duration = 0 في الـ
moov، // وChrome/Android ساعات يعتبر مدة أول fragment هي مدة الفيديو. //
هنا بنستخدم مدة YouTube الحقيقية ونكتبها في mvhd/tkhd/mdhd بدون إعادة
ترميز. function patchMp4MoovDuration(buffer, durationSeconds) { if
...
// أول box في output هو ftyp وبعده moov. let off = 0; while (off + 8 <=
...
durationSeconds = 0) { const UA = ‘Mozilla/5.0 (Windows NT 10.0; Win64;
x64) AppleWebKit/537.36 Chrome/120 Safari/537.36’; const inputArgs = [
...
‘copy’, // dash يضيف sidx للـ fragmented MP4، والـ moov بيتصلّح تحت قبل
الإرسال. ‘-movflags’, ‘frag_keyframe+empty_moov+default_base_moof+dash’,
‘-f’, ‘mp4’, ‘pipe:1’ ];
...
const flushPatchedHeader = () => { if (headerReady) return true; // الـ
moov عادة أول atom كبير؛ نستنى لحد ما يبقى كامل. let off = 0; while
...
if (headerBuffer.length > MAX_HEADER_BUFFER) { // حماية من تعليق الطلب
لو FFmpeg غير شكل الـ MP4. headerReady = true; res.write(headerBuffer);
...
cleanup(); if (!res.headersSent) res.status(500).json({ error: ‘ffmpeg
غير متاح على السيرفر’ }); }); ff.on(‘close’, code => { if (code !== 0 &&
...
!isValidVideoId(videoId)) { return res.status(400).json({ error: ‘Video
ID غير صحيح’ }); }
...
/** * POST /api/update-cookies */ app.post(‘/api/update-cookies’, async
(req, res) => { try { const secret = process.env.COOKIE_UPDATE_SECRET ||
’‘; const provided = req.get(’x-cookie-update-key’) ||
String(req.get(‘authorization’) || ’‘).replace(/^Bearer+/i,’‘); if
(!secret) return res.status(503).json({ error: ’COOKIE_UPDATE_SECRET غير
مضبوط على السيرفر’ }); if (!provided || provided !== secret) return
...
/** * GET /api/cookies-status */ app.get(‘/api/cookies-status’, async
(req, res) => { res.json({ hasCookies: cookiesReady, length:
lastCookiesContent.length, status: cookiesReady ? ‘✅ موجودة’ : ‘❌
فارغة أو غير موجودة’ }); });

/** * GET / */ app.get(‘/’, (req, res) => { res.json({ name: ‘🎬 srver
v7.0.0 “Turbo” - YouTube media server’, version: SERVER_VERSION,
environment: NODE_ENV, recommended:
‘/trending?region=EG&seed=dQw4w9WgXcQ’, cookies: { source: ‘🔥 Firebase
Realtime Database’, url: FIREBASE_URL, refresh: ‘كل 5 دقايق في الخلفية’,
ready: cookiesReady }, concurrency: { max: YTDLP_CONCURRENCY },
endpoints: { home: ‘/home?region=EG&perSection=12’, trending:
‘/trending?region=EG&limit=20&page=1’, video:
‘/video?v=VIDEO_ID&quality=1080 (أو &format=best للوضع السريع)’,
videoQualities: ‘/video/qualities?v=VIDEO_ID’, info: ‘/info?v=VIDEO_ID’,
formats: ‘/formats?v=VIDEO_ID’, search:
‘/search?q=QUERY&limit=20&page=1’, related:
‘/related?v=VIDEO_ID&limit=10&page=1’, channel:
‘/channel?id=CHANNEL_ID&limit=20&page=1’, comments:
‘/comments?v=VIDEO_ID&limit=50’, health: ‘/health’, cookiesStatus:
‘/api/cookies-status’ }, videoQualityValues: ‘يتم اكتشاف كل الارتفاعات
الحقيقية تلقائيًا عبر /video/qualities’, pagination: ‘كل endpoints
البحث/الترند/related/channel بترجع page و limit و hasMore — استخدمهم
لعمل infinite scroll’, examples: { ‘Home feed (أقسام زي يوتيوب)’:
‘/home?region=EG’, ‘Trending page 1’: ‘/trending?region=EG&page=1’,
‘Trending page 2 (سكرول لاحق)’: ‘/trending?region=EG&page=2’, ‘Play
video (جودة تلقائية)’: ‘/video?v=dQw4w9WgXcQ’, ‘Play video 1080p (دمج
ffmpeg)’: ‘/video?v=dQw4w9WgXcQ&quality=1080’, ‘Play video 4K’:
‘/video?v=dQw4w9WgXcQ&quality=2160’, ‘Audio only’:
‘/video?v=dQw4w9WgXcQ&quality=audio’, ‘كل الجودات المتاحة للفيديو ده’:
‘/video/qualities?v=dQw4w9WgXcQ’, ‘Search videos’:
‘/search?q=funny+cats&page=1’, ‘Related videos’:
‘/related?v=dQw4w9WgXcQ’, ‘Channel videos’:
‘/channel?id=UCuAXFkgsw1L7xaCfnd5JJOw’, ‘Video comments’:
‘/comments?v=dQw4w9WgXcQ’, ‘Check cookies’: ‘/api/cookies-status’ } });
});

// 404 app.use((req, res) => { res.status(404).json({ error: ‘Endpoint
غير موجود’, path: req.path }); });
...
// Start server const server = app.listen(PORT, ‘0.0.0.0’, () => { const
...
// Graceful shutdown process.on(‘SIGINT’, () => { log.warn(‘Shutting
down…’); server.close(() => { log.success(‘Server stopped’);
process.exit(0); }); });

process.on(‘SIGTERM’, () => { log.warn(‘Terminating…’); server.close(()
=> { process.exit(0); }); });
