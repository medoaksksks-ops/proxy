...
//   • Range support محسّن
// ==========================================================================
const SERVER_VERSION = '6.4.1-quality-fallback-minimal-diff';
...
/**
 * بيتنادى لما الجودة المطلوبة (زي 240/480) مش موجودة كـformat مستقل عند يوتيوب.
 * بيدوّر على أقرب جودة أعلى متاحة ويعمل لها scale بالـffmpeg للـheight المطلوب بالظبط.
 */
function streamScaledFallback(req, res, metadata, videoId, targetHeight, heights, startTime) {
  const maxNative = heights[0] || 0;
  if (targetHeight > maxNative) {
    return res.status(404).json({
      error: `الجودة ${targetHeight}p غير متاحة لهذا الفيديو`,
  ...
  const cacheKey = `stream_q_${videoId}_${targetHeight}`;
  const cached = streamCache.get(cacheKey);
  if (cached?.scaled) {
    log.info(`⚡ Scaled cached quality ${videoId}/${targetHeight}p startup=${Date.now() - startTime}ms`);
    return streamScaledViaFfmpeg(req, res, cached.videoUrl, cached.audioUrl, targetHeight);
  }

  const source = findClosestSourceForHeight(metadata, targetHeight);
  if (!source?.url) {
    return res.status(404).json({
      error: `تعذّر توفير الجودة ${targetHeight}p لهذا الفيديو حاليًا`,
      requestedQuality: targetHeight,
      availableQualities: heights.map(String),
      requestId: req.requestId
    });
  ...
  log.info(`🎯 Scaled ${videoId}/${targetHeight}p (من مصدر ${source.height}p) -> FFmpeg startup=${Date.now() - startTime}ms`);
  return streamScaledViaFfmpeg(req, res, source.url, audioForScale?.url || null, targetHeight);
}
...
app.get('/video', async (req, res) => {
  ...
  try {
    ...
    // Explicit quality: exact height only. لو مش موجودة أصلًا نعمل scale بدل ما نرفض الطلب.
    if (resolved?.type === 'video') {
      const heights = availableHeights(metadata);
      if (!heights.includes(resolved.height)) {
        // 🆕 الجودة (زي 240/480) مش موجودة كـformat مستقل عند يوتيوب → نعملها scale من أقرب جودة أعلى
        return streamScaledFallback(req, res, metadata, videoId, resolved.height, heights, startTime);
      }
